...
class CameraState:
    ...
    def __init__(self, index=0, alignment_rotation=0.0):
        ...
        self.prop = None
        self.index = index
        # Hardware (ASI driver) index this slot opens. USB enumeration order
        # can change between boots, so this is swappable independently of the
        # logical slot index (see CameraManager.swap_cameras).
...
class CameraManager:
    ...
    def __init__(self, num_cameras=2):
        ...
        self.SWAP_CAMERAS_BUTTON_RECT = None
    ...
    def swap_cameras(self, update_status_callback=None):
        ...
        USB enumeration order can change between boots, so the ASI driver
        sometimes hands back the cameras in swapped order. This exchanges the
        hardware indices of the first two slots and reconnects whichever were
        live, so each pane shows the other physical camera. Per-slot settings
        (gain/exposure/gamma/rotation) stay with the logical slot; ROI resets,
        as it would on any reconnect."""
        if len(self.cameras) < 2:
            return False

        cam0, cam1 = self.cameras[0], self.cameras[1]
        was_connected = [cam0.connected, cam1.connected]
        for i in range(2):
            if was_connected[i]:
                self.disconnect_camera(i)

        cam0.asi_index, cam1.asi_index = cam1.asi_index, cam0.asi_index

        ok = True
        for i in range(2):
            if was_connected[i]:
                ok = self.connect_camera(i, update_status_callback) and ok

        if update_status_callback:
            update_status_callback(
                f"Cameras swapped: Camera 1 = HW #{cam0.asi_index}, Camera 2 = HW #{cam1.asi_index}")
        return ok
