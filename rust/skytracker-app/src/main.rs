...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Fullscreen camera views: gamepad center pad (Mode button) cycles
        // guide → main → bubble → off; Esc (or the pad again past the last
        // camera, or the view's corner button) exits.
        let clicks = self.shared.fs_clicks.load(std::sync::atomic::Ordering::Relaxed);
        if clicks != self.ui.fs_seen {
            let presses = clicks.wrapping_sub(self.ui.fs_seen);
            self.ui.fs_seen = clicks;
            let n = self.shared.cams.len().max(1);
            for _ in 0..presses.min(8) {
                self.ui.fullscreen_cam = match self.ui.fullscreen_cam {
                    None => Some(0),
        ...
        if self.ui.fullscreen_cam.is_some() && ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
            self.ui.fullscreen_cam = None;
        }
        if let Some(fs) = self.ui.fullscreen_cam {
            let fs = fs.min(self.shared.cams.len().saturating_sub(1));
            egui::CentralPanel::default().frame(egui::Frame::none().fill(egui::Color32::BLACK)).show(ctx, |ui| {
                let h = ui.available_height();
                ui::camera_view(ui, &self.shared, &mut self.ui, fs, true, Some(h));
            });
            ctx.request_repaint();
            return;
        }
        ...
        match self.screen {
            Screen::Track => {
                ...
                match layout.as_str() {
                    ...
                    "stack" => {
                        // Explicit stored widths + our own drag handles: egui's
                        // side-panel resize memory snaps back (same failure as
                        // the bottom panels in the quad/tabs layouts).
                        let max_w = (ctx.screen_rect().width() * 0.45).max(340.0);
                        self.ui.stack_ctl_w = self.ui.stack_ctl_w.clamp(330.0, max_w);
                        self.ui.stack_cams_w = self.ui.stack_cams_w.clamp(280.0, max_w);
                        let ctl_w = self.ui.stack_ctl_w;
                        let cams_w = self.ui.stack_cams_w;
                        egui::SidePanel::right("right").resizable(false).exact_width(ctl_w).show(ctx, |ui| {
                            ui::left_edge_drag(ui, "stack_ctl_w", &mut self.ui.stack_ctl_w, 330.0, max_w);
                            ui::hdrag_handle(ui, "stack_ctl_w", &mut self.ui.stack_ctl_w, 330.0, max_w, true);
                        ...
                        egui::SidePanel::right("cams_col").resizable(false).exact_width(cams_w).show(ctx, |ui| {
                            ui::left_edge_drag(ui, "stack_cams_w", &mut self.ui.stack_cams_w, 280.0, max_w);
                            ui::hdrag_handle(ui, "stack_cams_w", &mut self.ui.stack_cams_w, 280.0, max_w, true);
                            let n = self.shared.cams.len().max(1);
