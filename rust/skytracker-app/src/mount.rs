            ...
            while let Some(ev) = g.next_event() {
                ...
                if let gilrs::EventType::ButtonPressed(b, _) = ev.event {
                    ...
                    match b {
                        ...
                        // Center pad (PS/Mode): cycle fullscreen camera views
                        // in the UI (guide → main → bubble → off).
                        Button::Mode => {
                            shared.fs_clicks.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        }
                        Button::Select => {
                            let idx = (bias_fine as usize) | ((bias_frame_ac as usize) << 1);
                            let next = (idx + 1) % 4;
                        ...
                        // Surface anything the map above missed so mismatched
                        // controller layouts are diagnosable in the field.
                        other => push_status(&mut status, format!("gamepad: unmapped button {other:?}")),
                    }
                }
            }
