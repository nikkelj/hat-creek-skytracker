...
pub struct UiState {
    ...
    /// Stack-layout column widths (far-right controls, camera column) —
    /// ours, not egui's (side-panel resize memory snaps back like the
    /// bottom panels did).
    pub stack_ctl_w: f32,
    ...
    /// Last-consumed value of Shared::fs_clicks.
    pub fs_seen: u32,
}
...
/// Full-height drag strip on a right side-panel's LEFT edge — the divider
/// grab the fingers expect. Dragging left widens the panel. Register it
/// FIRST inside the panel so content widgets keep pointer priority where
/// they overlap; the gutter between panels still lands here.
pub fn left_edge_drag(ui: &mut egui::Ui, id_salt: &str, val: &mut f32, min: f32, max: f32) {
    let pr = ui.max_rect();
    let edge = egui::Rect::from_min_max(
        Pos2::new(pr.left() - 6.0, pr.top()),
        Pos2::new(pr.left() + 6.0, pr.bottom()),
    );
    let resp = ui
        .interact(edge, ui.id().with((id_salt, "edge")), Sense::drag())
        .on_hover_cursor(egui::CursorIcon::ResizeHorizontal);
    if resp.hovered() || resp.dragged() {
        let c = if resp.dragged() { ACCENT } else { theme::with_alpha(ACCENT, 150) };
        ui.painter().line_segment(
            [Pos2::new(pr.left() - 1.0, pr.top()), Pos2::new(pr.left() - 1.0, pr.bottom())],
            Stroke::new(2.0, c),
        );
    }
    if resp.dragged() {
        *val = (*val - resp.drag_delta().x).clamp(min, max);
    }
}

/// Horizontal counterpart of `vdrag_handle`: a thin row at the top of a side
/// panel whose x-drag adjusts the panel's stored width. `invert` for right
/// panels (dragging left widens).
pub fn hdrag_handle(ui: &mut egui::Ui, id_salt: &str, val: &mut f32, min: f32, max: f32, invert: bool) {
    let (r, resp) = ui.allocate_exact_size(Vec2::new(ui.available_width(), 8.0), Sense::drag());
    ...
    for dx in [-4.0f32, 0.0, 4.0] {
        ui.painter().line_segment(
            [Pos2::new(r.center().x + dx, r.center().y - 3.0), Pos2::new(r.center().x + dx, r.center().y + 3.0)],
            Stroke::new(2.0, c),
        );
    }
    if resp.dragged() {
...
pub fn camera_view(ui: &mut egui::Ui, shared: &Arc<Shared>, st: &mut UiState, slot: usize, show_solve: bool, height: Option<f32>) -> Option<Rect> {
    ...
    // Fullscreen toggle: corner-bracket glyph, top-right (below the REC row).
    {
        let fs_on = st.fullscreen_cam == Some(slot);
        let b = Rect::from_min_size(r.rect.right_top() + Vec2::new(-26.0, 24.0), Vec2::splat(20.0));
        let resp = ui
            .interact(b, ui.id().with(("cam_fs", slot)), Sense::click())
            .on_hover_cursor(egui::CursorIcon::PointingHand)
            .on_hover_text(if fs_on { "exit fullscreen (Esc / center pad)" } else { "fullscreen this camera" });
        let col = if resp.hovered() || fs_on { ACCENT } else { theme::with_alpha(TEXT_2, 170) };
        let g = b.shrink(5.0);
        let k = 4.0f32.min(g.width() * 0.4);
        ...
        for (corner, dx, dy) in [
            ...
            p.line_segment([corner, corner + Vec2::new(dx, 0.0)], s);
            p.line_segment([corner, corner + Vec2::new(0.0, dy)], s);
        }
        if resp.clicked() {
            st.fullscreen_cam = if fs_on { None } else { Some(slot) };
        }
    }
    Some(img_rect)
}
